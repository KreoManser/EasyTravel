...
extension MainMenuViewController: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        case tripCollectionView:
            let vc = UIStoryboard(name: "EditTrip", bundle: nil)
            guard let editVC = vc.instantiateViewController(withIdentifier: "EditTripViewController") as? EditTripViewController else { return }
            let editNavVC = UINavigationController(rootViewController: editVC)
            
            editVC.trip = activeTrips[indexPath.row]
            editVC.delegate = self
            
            editNavVC.modalPresentationStyle = .fullScreen
            present(editNavVC, animated: true)
